...
CS：Code Segment代码段寄存器，用来存放代码段区域的入口地址（段基址）

ES：Extra Segment附加段寄存器，用来存放当前执行程序中一个辅助数据段的段地址

DS：Data Segment数据段寄存器，用来存放当前程序所使用数据段的最低地址

SS：Stack Segment堆栈段寄存器，用来存放堆栈段的基址
