...
覆盖技术指将程序按照逻辑上功能执行的独立性进行划分，将不可能在同一时间内执行的代码片段放在同一个空间上，使需要的时候再将相应模块加载进来

根据程序的功能划分模块

2. 什么是交换技术？覆盖与交换有什么不同？

交换的操作对象位进程，考虑的是一个进程整体的换入和换出

操作对象不同，覆盖操作的是程序内部的空间，而交换操作的是整个程序所占用的进程空间

3. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？

有用，在一些对内存要求非常严苛的场合需要覆盖和交换技术

4. 如何分析内核模块间的依赖关系？

查看代码的调用关系？查看依赖库？分析地址访问？不知道欸
...
时间局部性：一条指令的某次执行和下一次执行，一段数据的某次访问和下一次访问，往往都局限在某一段时间内

空间局部性：当前指令和邻近的几条指令所访问的数据往往都局限在某一段地址空间内

分支局部性：一条跳转指令的两次执行，往往会跳到相同的内存位置

2. 如何提高程序执行时的局部性特征？

修改代码时连续时间内的内存访问尽可能落在同一段空间当中
...
将不常用的部分放在外存中，只将现在需要的程序部分加载到内存当中

虚拟存储具有不连续性、大用户空间和部分交换的特征，这是覆盖和交换所不具备的

不连续性：物理地址的分配是不连续的，虚拟地址的使用是不连续的

大用户空间：用户可以实际使用的空间是要大于物理地址的空间的

部分交换：虚拟存储可以只交换程序使用的部分空间，不需要交换整个程序或者程序当中的整个模块

好处：上面三个

挑战：处理起来更为麻烦，在程序的局部性不好的时候性能很低

2. 虚拟存储需要什么样的支持技术？

硬件：页式或短时存储中的地址转换技术

操作系统：页式管理或者段式管理中页/段的换入和换出
